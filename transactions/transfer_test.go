...
func TestNewTransfer(t *testing.T) {
	...
	if transfer.TotalAmounts() != uint64(30) {
...
func TestValidateTransferData(t *testing.T) {
	...
	masterAddr2 := xmss.GetXMSSAddressFromPK(misc.UnSizedPKToSizedPK((masterXmssPK2[:])))
...
func TestSetAffectedAddress(t *testing.T) {
	ctrl := gomock.NewController(t)
	...
	mainChainMetaDataSerialized, _ := mainChainMetaData.Serialize()
	epochBlockHashesMetadata := metadata.NewEpochBlockHashes(epoch)
	otsIndex := uint64(binary.BigEndian.Uint32(transfer.pbData.Signature[0:4]))
	...
	err = transfer.SetAffectedAddress(stateContext)
	if err != nil {
		t.Error("got unexpected error while setting affected addresses ", err)
	}
}

func TestGetSigningHash(t *testing.T) {
	// masterXmssPK, _ := hex.DecodeString("00030001e9460db4c459269f5545524d78da462945821ff47e5191b9664735b071e18f5fec7dcb78e39a9ad6f075b7c6ef629325bd716f9719a452362744880b5341e7")
	...
	expectedHash, _ := hex.DecodeString("6533ffa8f20d21ea3542c7caca4cc1bf38c64515eef81f649ae85f06ed0ab29c")

	output := transfer.GetSigningHash()
	if hex.EncodeToString(output) != hex.EncodeToString(expectedHash) {
		t.Errorf("expected transfer signing hash to be (%v), got (%v)", hex.EncodeToString(expectedHash), hex.EncodeToString(output))
	}
}
