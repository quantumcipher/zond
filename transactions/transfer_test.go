...
func TestNewTransfer(t *testing.T) {
	...
	networkID := uint64(1)
	data := []byte("data")
	transfer := NewTransfer(networkID, addrTo[:], amount, gas, gasPrice, data, nonce, masterXmssPK[:])
...
func TestGetSigningHash(t *testing.T) {
	...
	expectedHash := "0x6fb6e4a05a80bbc6e957983ab253a86ece3862e3450ffe114659010246c1d45c"

	output := transfer.GetSigningHash()
	if output.String() != expectedHash {
		t.Errorf("expected transfer signing hash to be (%v), got (%v)", expectedHash, output.String())
	}
}
