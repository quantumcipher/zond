...
func TestValidateData(t *testing.T) {
	...
	dbAddressStateKey := address.GetAddressStateKey(binCoinBaseAddress[:])
...
func TestCoinbaseValidate(t *testing.T) {
	...
	coinbase.Sign(blockProposer, coinbase.GetSigningHash(stateContext.BlockSigningHash()))
...
func TestCoinbaseSetAffectedAddress(t *testing.T) {
	ctrl := gomock.NewController(t)
	...
	addressState := address.NewAddressState(binCoinBaseAddress[:], 10, 20000000000001)
	blockproposerAddressState := address.NewAddressState(blockProposerAddr[:], 10, 20001)
	blockproposerAddressStateSerialized, _ := blockproposerAddressState.Serialize()
	addressesState := make(map[string]*address.AddressState)
	addressesState[hex.EncodeToString(binCoinBaseAddress[:])] = addressState
	addressesState[hex.EncodeToString(blockProposerAddr[:])] = blockproposerAddressState
	addressesStateSerialized, _ := addressState.Serialize()
	...
	store.EXPECT().Get(gomock.Eq(metadata.GetEpochBlockHashesKey(epoch))).Return(epochBlockHashesMetadataSerialized, nil).AnyTimes()
	store.EXPECT().GetFromBucket(gomock.Eq(dbAddressStateKey), gomock.Eq(metadata.GetBlockBucketName(parentBlockHeaderHash))).Return(addressesStateSerialized, nil).AnyTimes()
	store.EXPECT().GetFromBucket(gomock.Eq(address.GetAddressStateKey(blockProposerAddr[:])), gomock.Eq(metadata.GetBlockBucketName(parentBlockHeaderHash))).Return(blockproposerAddressStateSerialized, nil).AnyTimes()
	store.EXPECT().GetFromBucket(gomock.Eq(blockproposerMetadataKey), gomock.Eq(metadata.GetBlockBucketName(parentBlockHeaderHash))).Return(blockProposerDilithiumMetadataSerialized, nil).AnyTimes()
	...
	err = coinbase.SetAffectedAddress(stateContext)
	if err != nil {
		t.Error("got unexpected error while applying state changes in coinbase transaction ", err)
	}
}
...
func TestCoinbaseGetSigningHash(t *testing.T) {
	...
	expectedHash, _ := hex.DecodeString("647d90030df07189621686f68b052ee8605d9865f8f60d736ba1829cb64e4ea8")
	output := coinbase.GetSigningHash(stateContext.BlockSigningHash())

	if hex.EncodeToString(output) != hex.EncodeToString(expectedHash) {
		t.Errorf("expected coinbase signing hash (%v), got (%v)", hex.EncodeToString(expectedHash), hex.EncodeToString(output))
	}
}
...
func TestCoinbaseUnsignedHash(t *testing.T) {
	...
	if hex.EncodeToString(output) != hex.EncodeToString(expectedHash) {
		t.Errorf("expected coinbase unsigned hash (%v), got (%v)", hex.EncodeToString(expectedHash), hex.EncodeToString(output))
	}
}
